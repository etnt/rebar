...
%%
%% @doc Eunit reporter, produces HTML (or Eterm output).
%%
...
-record(state, {verbose     = false,
		indent      = 0,
		xmldir      = ".",
                format      = html,  % html | eterm | surefire | all
                options     = [],
                application = "",
		testsuite   = #testsuite{},
...
handle_begin(group, Data, St) ->
    %%error_logger:info_msg("+++ HANDLE_BEGIN: ~p~n",[Data]),
    NewId = proplists:get_value(id, Data),
    case NewId of
	[] ->
            %%error_logger:error_msg("~p(~p): no Id found~n",[?MODULE,?LINE]),
	    St;
...
handle_end(group, Data, St) ->
    %%error_logger:info_msg("+++ HANDLE_END: ~p~n",[Data]),
...
hwrite_report_index(TSs, Result, App, XmlDir) ->
    ...
    Ehtml = {html,[],
             [{head,[],[css()]},
              {body,[],
               [{h2,[],["Test suite for the "++to_string(App)++" application"]},
